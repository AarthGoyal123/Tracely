...
- **First time?** → Read [DEMO.md](./DEMO.md)
- **Deep dive?** → Read [ARCHITECTURE.md](./ARCHITECTURE.md)
- **Presenting?** → Read [PITCH.md](./PITCH.md)
...
A: No! Use [MongoDB Atlas](https://www.mongodb.com/cloud/atlas) (free tier). Just update `.env` with your connection string.

**Q: Why is the extension detecting nothing on my sites?**  
A: Make sure:
   - Backend is running (`npm run dev` in backend folder)
   - Extension has permissions (check `chrome://extensions/`)
   - You're visiting a site with third-party requests
   - Service Worker console shows no errors (`chrome://extensions/` → Tracely → Service Worker)

**Q: Can I modify mock data?**  
A: Yes! Edit frontend pages in `/frontend/src/pages/` and change mock site data.

**Q: How do I see real tracking?**  
A: 
1. Start backend (`cd backend && npm run dev`)
2. Load extension in Chrome
3. Visit any popular website (news, social media, etc.)
4. Open extension popup to see detected trackers in real-time
5. Visit dashboard at `http://localhost:5173` to see aggregated data
6. Check Analytics page for tracker distribution visualization

**Q: What's the difference between "My Data" and "Global Stats"?**  
A: 
- **My Data**: Shows only trackers you personally detected with the extension
- **Global Stats**: Shows aggregated data from all users (when authentication is enabled)
- Toggle between them using the button on the Dashboard

**Q: Is this production-ready?**  
A: It's a hackathon MVP! For production:
- Add error handling
